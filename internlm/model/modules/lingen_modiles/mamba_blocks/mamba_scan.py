...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def backward(ctx, dout):
        ...
        ### no output projection at the end of forward
...
class BiMambaInnerFn(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        assert not A_b.is_complex(), "A_b should not be complex"
        out_b, scan_intermediates_b, out_z_b = selective_scan_cuda.fwd(
            ...
            C.flip([-1]), 
            D, 
            z.flip([-1]), delta_bias, delta_softplus
        )
        ...
        ctx.save_for_backward(xz, conv1d_weight, conv1d_bias, x_dbl, x_proj_weight,
                              delta_proj_weight, out_proj_weight, conv1d_out, delta,
                              A, B, C, D, delta_bias, scan_intermediates_f, scan_intermediates_b,
    ...
    def backward(ctx, dout):
        ...
        d_conv1d_out_f_b, ddelta_f_b, dA_b, dB_f_b, dC_f_b, dD_b, ddelta_bias_b, dz_b, out_z_b = selective_scan_cuda.bwd(
            conv1d_out.flip([-1]), delta.flip([-1]), A_b, B.flip([-1]), C.flip([-1]), D, z.flip([-1]), delta_bias, dout_y.flip([-1]), 
            scan_intermediates_b, out_b, dz_b,
            ctx.delta_softplus,
            True  # option to recompute out_z
        )

        dconv1d_out = dconv1d_out + d_conv1d_out_f_b.flip([-1])
