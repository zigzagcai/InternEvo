...
class ProcessGroupSingleton(Singleton):
    def __init__(self):
        ...
        self.P2P_PG = None
        self.DKV_ALLGATHER_PG = None
        self.DKV_P2P_PG = None
...
def get_sliding_window_pg(rank, window_num, window_size, ring_ranks, interleaved):
    ...
    all_gather_pg, p2p_pg, dkv_all_gather_pg, dkv_p2p_pg = None, None, None, None
    ...
    if interleaved is False:
        ...
        for j in range(window_num):
            ...
            dkv_group = torch.distributed.new_group(window_ranks)
            if rank in window_ranks:
                all_gather_pg = group
                dkv_all_gather_pg = dkv_group
        ...
        for j in range(window_size):
            ...
            dkv_group = torch.distributed.new_group(p2p_ranks)
            if rank in p2p_ranks:
                p2p_pg = group
                dkv_p2p_pg = dkv_group
    ...
    return all_gather_pg, p2p_pg, dkv_all_gather_pg, dkv_p2p_pg


def set_seq_parallel_pg(
    sp_ulysses_degree, sp_ring_degree, rank, world_size, use_ulysses_low=True, window_size=1, interleaved=False
):
    """
    ...
    if use_ulysses_low:
        for dp_rank in range(dp_degree):
            ...
            for i in range(num_ring_pgs):
                ...
                all_gather, p2p, dkv_all_gather, dkv_p2p = get_sliding_window_pg(
                    rank, window_num, window_size, ring_ranks, interleaved
                )
                if all_gather is not None:
                    all_gather_pg = all_gather
                if p2p is not None:
                    p2p_pg = p2p
                if dkv_all_gather is not None:
                    dkv_all_gather_pg = dkv_all_gather
                if dkv_p2p is not None:
                    dkv_p2p_pg = dkv_p2p
    ...
    PROCESS_GROUP.DKV_ALLGATHER_PG = dkv_all_gather_pg
    PROCESS_GROUP.DKV_P2P_PG = dkv_p2p_pg
